...
try {
    $clanShortId = $_POST["c"];
    $clanShortIdLower = strtolower($clanShortId);
    $result = searchClanByTag($clanShortIdLower);

    if (count($result) == 0) {
        $clanShortIdUpper = strtoupper($clanShortId);
        $result = searchClanByTag($clanShortIdUpper);

        if (count($result) == 0) {
            throw new Error("Clan [$clanShortIdLower] and [$clanShortIdUpper] not found.");
        }
    }
    ...
    while (true) {
        ...
        if (count($result) == 1500) {
            $offset += 1500;
            $result = clanMessages($clanId, $offset);
            $allResult = array_merge($allResult, $result);
        } else {
            break;
        }
...
function searchClanByTag($clanShortName) {
    // Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
    $ch = curl_init();

    curl_setopt($ch, CURLOPT_URL, "https://mfrgjyclqnoizevhylbh.supabase.in/rest/v1/clans?select=*&order=level.desc.nullslast&shortName=eq.{$clanShortName}&limit=1");
...
function clanMessages($clanId, $offset = 0) {
    // Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
    $ch = curl_init();

    curl_setopt($ch, CURLOPT_URL, "https://mfrgjyclqnoizevhylbh.supabase.in/rest/v1/chat_messages?select=*&channel=in.%28{$clanId}%29&order=time.desc.nullslast&limit=1500&offset={$offset}");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
